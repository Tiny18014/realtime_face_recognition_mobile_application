...
Future<void> main() async {
  ...
  bool hasRegisteredFace = await _checkFaceRegistration();

  runApp(MyApp(hasRegisteredFace: hasRegisteredFace));
}

Future<bool> _checkFaceRegistration() async {
  ...
  if (user != null) {
    final doc = await FirebaseFirestore.instance
        ...
        .doc(user.uid)
        .get();
    return doc.exists && (doc.data()?['hasFaceRegistered'] ?? false);
  }
  return false;
}
...
class MyApp extends StatelessWidget {
  ...
  const MyApp({super.key, required this.hasRegisteredFace});

  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      debugShowCheckedModeBanner: false,
      home: hasRegisteredFace ? const HomeScreen() : FaceRegistrationScreen(),
    );
  }
}

class FaceRegistrationScreen extends StatefulWidget {
  FaceRegistrationScreen({Key? key}) : super(key: key);
  @override
...
class _FaceRegistrationScreenState extends State<FaceRegistrationScreen> {
              ...
              ElevatedButton(
                onPressed: () async {
                  try {
                    final user = FirebaseAuth.instance.currentUser;
                    if (user != null) {
                      // Save face embeddings in Firestore
                      await FirebaseFirestore.instance
                          .collection('users')
                          .doc(user.uid)
                          .set({
                        'name': textEditingController.text,
                        'faceEmbeddings': recognition.embeddings,
                        'hasFaceRegistered': true,
                        'email': user.email,
                      }, SetOptions(merge: true));

                      // Register face in local DB as well
                      ...
                      String role =
                          userDoc['role']; // Assuming 'role' field exists
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content:
                            Text("Error registering face. Please try again."),
  ...
  Future<void> _updateFaceRegistrationStatus() async {
    final user = FirebaseAuth.instance.currentUser;
    if (user != null) {
      try {
  ...
  Future<void> checkExistingFaceRegistration() async {
    final user = FirebaseAuth.instance.currentUser;
    if (user != null) {
      try {
        final doc = await FirebaseFirestore.instance
        ...
        if (doc.exists &&
          ...
          // User already has face embeddings stored, navigate to home
